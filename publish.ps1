...
#
# Usage: close the Unity editor, then run this in PowerShell:
#   ./publish.ps1
#
...
if (Test-Path "$project\Temp\UnityLockfile") {
    Write-Host "Close the Unity editor first — the project is locked while it's open." -ForegroundColor Yellow
    exit 1
}
...
& $unity -quit -batchmode -nographics -projectPath $project `
    -buildTarget Win64 -buildWindows64Player "$out\ClutchFPS.exe" -logFile -
if ($LASTEXITCODE -ne 0) { Write-Host "Build failed." -ForegroundColor Red; exit 1 }

# Drop the debug-symbols folder that shouldn't ship, then zip.
